...
$my = "24.10.1982";
$today = date("d.m.Y");
$razn = (strtotime($today) - strtotime($my)) / (60 * 60 * 24); //Продолжение в футере строка 113
...
настоящее время работаю на одном из предприятий Уральской горно-металлургической компании. Не судим. Женат. Двое детей.</p>"; //Далее стр 55
...
<p>Давайте продолжим!</p>"; //Далее стр 63
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<section class="content">
    ...
    <section class="name <? echo $now[2] ?>"><? echo $name = "Сергеев Сергей"; ?></section>
    <section class="about_me <? echo $now[2] ?>">
        <? $anotherColor = substr($about_me, 0, strpos($about_me, ".", 0));
        $about = strstr($about_me, ".");
        echo "<span style='color: #800f00'>$anotherColor</span>";
        echo "<span style='color: black'>$about</span>";
        ?>
    </section>

    <section class="about_lessons <? echo $now[2] ?>">
        <? $arr = explode(" ", $about_lesson);
...
<footer class="footer">
    <section class="<? echo $now[2] ?>">
        ...
        <p><? echo $today; ?> - текущее значение.</p>
        <p>Разность - <? echo floor($razn); ?> дня.</p>
        <hr>
        <? //Задача про количество слов
        $summWord = [$about_lesson, $about_me, $elem1, $elem2, $elem3, $elem4, $elem5, $elem6, $elem7, $elem8, $name];
        $summ = 0;
        foreach ($summWord as $str) {
            $summ += count(explode(" ", $str));
        }
        echo "<p>Всего слов на странице: $summ </p>";

        //Задача про гласные
        $sumGlas = 0;
        foreach ($summWord as $string) {
            $string = mb_strtolower($string);
            $arrGlas = ['я', 'о', 'э', 'е', 'и', 'ы', 'у', 'ё', 'ю', 'а'];
            foreach ($arrGlas as $var) {
                $sumGlas += substr_count($string, $var);
            }
        }
        echo "<p>Всего гласных на странице: $sumGlas</p>";

        // Еще один способ узнать количество гласных
        /*$str_glas = implode(" ", $summWord);
